...
![tests](https://img.shields.io/badge/integration-123%2F123%20green-brightgreen)
...
| [`madobi-verifier`](.claude/agents/madobi-verifier.md) | **보고서 숫자 적대 검증 전담** — Doer–Verifier 의 Verifier. 중요 보고서는 `APPROVED` 후에만 발송 |
...
**서비스 프롬프트도 측정한다:** 에이전트/스킬 프롬프트는 [`research/prompt_eval/`](research/prompt_eval/) 의
**행동계약 10종**(검산 우선 · Simpson 미끼 거절 · 모호하면 1개만 되묻기 · 숫자 날조 금지 · 소표본 경고 …)으로
회귀 평가한다 — `build_eval.py` 가 madobi vs baseline 2-arm 비교 워크플로를 생성하고, `grade.py` 가
**결정론 채점**(regex/길이 assertion — LLM 채점 없음). 세션 시작 훅이 하네스 상태를 한 줄로 브리핑한다.
...
- `marketing/bench/`가 **10만 행 × 13필드**(콤마·₩·혼합 포맷) 데이터와 **코드로 계산한 정답**을 만들고,
  **40단계 난이도 사다리**로 집계를 검증 (시간윈도우 경계·중복제거·멀티통화 함정 포함) — 단일·**멀티 피벗(말도 안 되는 4중 피벗 225그룹 포함)**,
...
=== GRADED BENCHMARK: 40/40 levels PASS ===
...
## 💬 챗봇 모드 — 자연어로 물어보면 도구로 연결

클로드 없이도 터미널에서 바로 대화합니다. 라우팅은 **결정론**(LLM 호출 없음, 같은 질문 → 같은 답):

```text
$ python marketing/madobi.py chat
매도비> 이 CSV 숫자 검산해줘: data/campaign.csv
추천 도구:
  1. reconcile — 정합성 검산 …
     실행: python marketing/reconcile.py data/campaign.csv
매도비> run                                  # 추천 1번을 그 자리에서 실행
매도비> ROAS가 뭐야?
[ROAS] 광고비 대비 매출 = 매출/비용  공식: revenue / spend …
```

- **지표 Q&A**: "CPA가 뭐야?" → 시맨틱 레이어 용어집에서 정의·공식·SQL식 즉답
- **도구 라우팅**: 한국어 의도(검산/예산/이탈/코호트 …) → 89개 도구 중 최적 추천 + **검증된 실행 명령**
  (예시 인자는 `tests/run_all.py`의 통과된 호출을 그대로 재사용 — 이중 정의 없음)
- **놀리지 회상**: "지난번에 배운 교훈?" → `knowledge/recall.py` 검색 위임
- 단발 질의는 `python marketing/chat.py --ask "질문"` (스크립트/에이전트 연동용)
...
```
.claude/agents/   madobi.md, madobi-verifier.md, smartest.md   # 제품 에이전트 (Doer–Verifier 분리)
.claude/skills/   marketing-analyst/SKILL.md      # 루틴 자동화 스킬
.claude/settings.json                             # SessionStart 브리핑 + Stop 훅(autosync)
marketing/        STRATEGY · METRICS · CREATIVE_PLAYBOOK · MEMORY_PROTOCOL · LEARNING_LOOP
                  madobi.py(디스패처) · chat.py(챗봇 REPL) · reconcile.py · summarize · analyze_creatives
                  pm/(퍼마 도구 78개)
                  knowledge/(213 자산) · bench/(집계+추론 벤치) · samples
...
research/prompt_eval/  서비스 프롬프트 행동계약 평가 (측정→패치→재측정 루프, 결정론 채점)
```
...
cross-checked by **deterministic Python** (`reconcile.py`, `bench/summarize.py`, the 78 tools in `pm/`); the LLM
...
**40-level difficulty ladder** (incl. time-window boundary, dedup and multi-currency traps) — single and **multi-pivot (incl. an absurd 4-dimension / 225-group pivot)**,
...
The 213 knowledge assets are searchable via **SQLite FTS5 + bm25 ranking** (`marketing/knowledge/search.py`) —
