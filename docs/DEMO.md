...
샘플 캠페인 데이터([`marketing/samples/sample_campaign.csv`](../marketing/samples/sample_campaign.csv), 오류를 일부러 심음)에
MADOBI 도구를 돌린 **실제 결과**입니다. 모두 코드로 생성되며 재현 가능합니다.

## 1. 정합성 검산 — 숨은 숫자 오류 적발
```bash
python marketing/reconcile.py marketing/samples/sample_campaign.csv --md docs/demo/reconcile_report.md
```
...
## 2. 소재 분석 — 승자/패자 자동 식별
```bash
python marketing/analyze_creatives.py marketing/samples/sample_campaign.csv --kpi roas --min-impr 1000 --md docs/demo/creative_report.md
```
...
## 3. 집계 요약 — 가중지표로 정확히
```bash
python marketing/bench/summarize.py marketing/samples/sample_campaign.csv --by creative --md docs/demo/summary_by_creative.md
```
