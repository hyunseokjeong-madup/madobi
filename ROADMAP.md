...
대화에서 나온 모든 요구를 포인트로 정리 (개수는 표가 진실 — 계속 자란다). ✅완료 / 🚧진행 / ⬜예정. 위에서부터 천천히 루프.
...
| 4 | **대용량·다필드 집계 (코드검증 벤치 40/40)** | ✅ | `bench/` 10만행×13필드, 40단계(4D 피벗+함정) 전부 PASS |
...
| 7 | **난이도 50단계 확장 + 함정 케이스** | 🚧 | 시간윈도우·멀티통화·중복제거·결측셀·0분모 함정 11종 반영 (29 → 40). 코호트·어트리뷰션 레벨은 잔여 (40 → 50+) |
| 8 | **리포트 자동 생성기**(일/주/월) | ✅ | `marketing/report.py` — 검산 포함 성과 리포트 마크다운 |
| 8b | **퍼마 일상 루틴 도구**(페이싱/퍼널/AB/재배분…) | ✅ | `marketing/pm/*` 78종 — 300사이클 완료 트래커 `CYCLES.md` (이후 확장은 별도 항목으로) |
| 9 | **마케팅 추론 벤치 확대 + 함정** | 🚧 | `bench/reasoning4.py` 함정 8종(Simpson·어트리뷰션 중복·타임존 경계·베이즈 사후) — 정답 닫힌형식 계산·trap/why 주석. 추가 확대 여지 |
| 10 | **온보딩 + E2E 데모 + CI** | ✅ | ✅계정 온보딩(`curate.py --onboard`) · ✅CI(`.github/workflows/ci.yml`, 통합 체크 전체+집계 벤치 전 레벨, py3.9/3.12 + compileall 구문가드) · ✅E2E 데모(`marketing/demo_e2e.py` 회상→검산→집계→삼중검증→루프 라운드트립, `--selftest` 내장) |
...
| 12 | **RAG ↔ RL 시너지 폐곡선** | ✅ | ✅`recall.recall_quality()`(회상 품질=색인 hit/grep·매칭비율, 100% 코드산물) → bandit 보상 · ✅결정론 confidence 배지 · ✅`viz.py` 그래프 JSON(노드208·에지680) · ✅`loop_report.py` observer(없는 산술 날조 안 함) |
